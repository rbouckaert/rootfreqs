...
@Description("Beagle Tree-likelihood that allows site specific root frequencies")
public class TreeLikelihoodWithError extends beast.base.evolution.likelihood.TreeLikelihood {

    protected BeagleTreeLikelihoodWithError beagleWithError;
    ...
    public void initAndValidate() {
        ...
        beagleWithError = null;

        beagleWithError = new phylonco.beast.evolution.likelihood.BeagleTreeLikelihoodWithError();

        try {
            beagleWithError.initByName(
                    ...
                    "errorModel", errorModelInput.get(), "useTipsEmpirical", useTipsEmpiricalInput.get());

        } catch (Exception e) {
            // exit
            throw new RuntimeException("No Beagle instance was found, aborting!");
        }
    ...
    public double calculateLogP() {
        if (beagleWithError != null) {
            logP = beagleWithError.calculateLogP();
            if (rootFrequenciesSequence != null) {
                logP = recalculateBeagleLogPWithRootFrequences();
            }
        } else {
            System.out.println("BeagleWithError is null at calculateLogP");
    ...
    private double recalculateBeagleLogPWithRootFrequences() {
        ...
        int node = beagleWithError.getPartialBufferHelper().getOffsetIndex(number);
        if (categoryCount <= 1) {
            beagleWithError.getBeagle().getPartials(node, Beagle.NONE, rootpartials);
        } else {
            beagleWithError.getBeagle().getPartials(node, Beagle.NONE, rootpartials2);
        ...
        double [] beaglePatternLogLikelihoods = beagleWithError.getPatternLogLikelihoods();
    ...
    public double [] getPatternLogLikelihoods() {
        if (beagleWithError != null) {
            return beagleWithError.getPatternLogLikelihoods();
        }
        return patternLogLikelihoods.clone();
    } // getPatternLogLikelihoods

    @Override
    // defined here to provide access for rootfreqs.ThreaderTreeLikelihood and beagleWithError
    protected boolean requiresRecalculation() {
        if (beagleWithError != null) {
            return beagleWithError.requiresRecalculation();
        }
    ...
    public void store() {
        if (beagleWithError != null) {
            beagleWithError.store();
            return;
        } else {
            System.out.println("beagleWithError is null in TreeLikelihoodWithErrorRootSeq store()");
        }
